        ...
        <div class="max-w-7xl mx-auto space-y-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
                <div class="rounded-2xl bg-[color:var(--white)] border border-[color:var(--cream-dark)] p-5 shadow-sm">
                    <p class="text-xs font-semibold uppercase tracking-wider text-[color:var(--ink-muted)]">{{ __('Pagos pendientes') }}</p>
                    <p class="mt-2 text-3xl font-semibold text-[color:var(--ink)]">{{ number_format($pendingPaymentsCount) }}</p>
                </div>
                <div class="rounded-2xl bg-[color:var(--white)] border border-[color:var(--cream-dark)] p-5 shadow-sm">
                    <p class="text-xs font-semibold uppercase tracking-wider text-[color:var(--ink-muted)]">{{ __('Pagos completados') }}</p>
                    <p class="mt-2 text-3xl font-semibold text-[color:var(--ink)]">{{ number_format($paidPaymentsCount) }}</p>
                </div>
                <div class="rounded-2xl bg-[color:var(--white)] border border-[color:var(--cream-dark)] p-5 shadow-sm sm:col-span-2">
                    <p class="text-xs font-semibold uppercase tracking-wider text-[color:var(--ink-muted)]">{{ __('Monto cobrado') }}</p>
                    <p class="mt-2 text-3xl font-semibold text-[color:var(--ink)]">${{ number_format($totalPaidAmount, 2) }}</p>
                </div>
            </div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-[color:var(--cream-dark)]">
                        ...
                        <tbody class="divide-y divide-[color:var(--cream-dark)]">
                                ...
                                <tr class="hover:bg-[color:var(--cream)] transition">
                                    ...
                                    <td class="px-5 py-4">
                                        @if ($payment->status === 'paid')
                                            <span class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold" style="background: var(--success-bg); color: var(--success-text);">{{ __('Pagado') }}</span>
                                        @elseif ($payment->status === 'pending')
                                            <span class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold" style="background: var(--warning-bg); color: var(--warning-text);">{{ __('Pendiente') }}</span>
                                        @else
                                            <span class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold" style="background: var(--danger-bg); color: var(--danger-text);">{{ ucfirst((string) $payment->status) }}</span>
                                        @endif
                                    </td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-5 py-10 text-center text-sm text-[color:var(--ink-muted)]">{{ __('No hay pagos para mostrar.') }}</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
